...
async function initDB() {
    try {
        // 1. PATIENTS (Parent Table)
        // Must have UNIQUE on patient_id to allow other tables to link to it
        await pool.query(`
        ...
        // 2. PATIENT_BILLING (Child Table)
        // Type MUST be VARCHAR(20) to match patients.patient_id exactly
        await pool.query(`
            CREATE TABLE IF NOT EXISTS patient_billing (
                id SERIAL PRIMARY KEY,
                patient_id VARCHAR(20) REFERENCES patients(patient_id) ON DELETE CASCADE,
                services_taken TEXT,
                amount_per_session DECIMAL(10, 2),
        ...
        await pool.query(`CREATE TABLE IF NOT EXISTS doctor (
            id SERIAL PRIMARY KEY, name TEXT, qualification TEXT, image_url TEXT, phone TEXT
        )`);

        await pool.query(`CREATE TABLE IF NOT EXISTS feedbacks (
            id SERIAL PRIMARY KEY, patient_name TEXT, mobile TEXT, details TEXT, rating INTEGER
        )`);
        ...
        await pool.query(`
            ...
            VALUES ('meghanap142@gmail.com', 'Megha@4516')
            ON CONFLICT (username) DO NOTHING
        `);

        console.log(' Database connected and all tables initialized correctly.');
    } catch (err) {
...
app.get('/forgot-password', (req, res) => {
    res.send(`
        <div style="font-family: sans-serif; max-width: 400px; margin: 100px auto; padding: 25px; border: 1px solid #ddd; border-radius: 10px; text-align: center;">
            ...
            <p>A reset link will be sent to meghanap142@gmail.com</p>
...
app.post('/api/send-reset', async (req, res) => {
 ...
 app.get('/about-doctor', (req, res) => {
    ...
    <head>
        ...
        <title>Dr. Mehana P - Profile</title>
    ...
    <body>
        <div class="info-card">
            <img src="/doctor.jpg" alt="Dr. Meghana P PT" class="profile-large">
            <h1>Dr. Meghana P</h1>
            ...
            <div class="details-grid">
                ...
                <div class="detail-item full-width">
                    ...
                    <p>Sports Injury Specialist, Restoring basic mobility, managing chronic pain (back/neck), and complex rehabilitation after surgery or illness.</p>
                </div>
...
app.get('/electro', (req, res) => {
    ...
    <head>
        <meta charset="UTF-8">
        <title>Electrotherapy - Meghana Physio Care & Home Services </title>
        <link href="https://fonts.googleapis.com/css2?family=Poppins:wght@300;400;600;700&display=swap" rel="stylesheet">
        <style>
...
app.get('/injury-prevention', (req, res) => {
    ...
    <head>
        <meta charset="UTF-8">
        <title>Injury Prevention - Meghana Physio Care & Home Services</title>
        <link href="https://fonts.googleapis.com/css2?family=Poppins:wght@300;400;600;700&display=swap" rel="stylesheet">
        <style>
            body { font-family: 'Poppins', sans-serif; line-height: 1.6; color: #1f2937; max-width: 800px; margin: 40px auto; padding: 20px; background: #f3f6f9; }
            .info-card { background: white; padding: 40px; border-radius: 24px; box-shadow: 0 10px 30px rgba(0,0,0,0.05); }
...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Meghana Physio Care & Home Services</title>
...
/* Update your header/banner container class */
.header-banner {
    display: flex;
    ...
    text-align: center;
    background: #1e293b; /* Use your dark primary color here */
    padding: 40px 20px 80px 20px; /* Adjust padding to fit the doctor image overlay */
    width: 100%;
    box-sizing: border-box;
    border-radius: 0 0 30px 30px; /* Optional: adds the curved bottom effect */
    margin: 0;
}
