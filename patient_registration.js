...
async function sendTwilioSMS(patientName, patientId, patientMobile) {
    try {
        ...
        await client.messages.create({
            body: `Welcome ${patientName}! Registration successful at Meghana Physio Care. Patient ID: ${patientId}.`,
            from: twilioPhone,
            to: cleanMobile
        });
...
module.exports = (pool) => {
    ...
    router.post('/api/register-patient', async (req, res) => {
        try {
            ...
            res.send(`
                <script>
                    alert("Registration Successful! Patient ID: ${finalId}\\n\\nSMS has been sent to Patient & Admin.");
                    window.location.href = "${adminWaLink}";
                </script>
            `);
