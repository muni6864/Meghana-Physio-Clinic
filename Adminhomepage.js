...
<body>
    <div class="container">
        <div class="header">
            <div>
                <h1>Meghana Physio Care & Home Services</h1>
...
<input type="text" value="meghanap142@gmail.com" disabled style="background:#f1f5f9; color:#475569; cursor:not-allowed; font-weight:bold;">
...
<div id="modalOverlay"><div class="modal"><h3>Treatment Profile</h3><div id="modalContent" style="background:#f1f5f9; padding:20px; border-radius:12px; margin-bottom:20px; font-size:14px;"></div><form action="/admin/update-treatment" method="POST"><input type="hidden" name="id" id="m_id"><div style="display:flex; gap:15px;"><div style="flex:1;"><label>Start Date</label><input type="date" name="start_date" max="${today}" id="m_start"></div><div style="flex:1;"><label>End Date</label><input type="date" name="end_date" max="${today}" id="m_end"></div></div><label>Update Status</label><select name="status" id="m_status"><option value="Patient Considered">Patient Considered</option><option value="Treatment Started">Treatment Started</option><option value="Completed & Payment Pending">Completed & Payment Pending</option><option value="Completed & Payment Received">Completed & Payment Received</option><option value="Closed">Closed</option></option><option value="Cancelled">Cancelled</option></select><label>Notes</label><textarea name="t_details" id="m_details" rows="4"></textarea><button type="submit" style="background:var(--success); width:100%; padding:15px; font-size:16px;">SAVE RECORD</button></form><button class="btn-delete" style="width: 100%; margin-top: 10px; background: #64748b;" onclick="closeModal()">CLOSE</button></div></div>
    ...
    <div class="modal" style="background:white; padding:30px; border-radius:15px; width:600px; position:absolute; top:50%; left:50%; transform:translate(-50%, -50%);">
        <center><h2>Meghana Physio Care & Home Services</h2><p>Billing Dashboard</p></center>
        <hr>
    ...
    async function openPaymentHistoryModal(patientId, patientName) {
        ...
        try {
            ...
            history.forEach(row => {
                ...
                // Notice the backslashes below: \` and \$
                tbody.innerHTML += \`
                    <tr>
                        <td>\${date}</td>
    ...
    function printPaymentHistory() {
        ...
        area.innerHTML = 
            '<div style="padding:40px; font-family: sans-serif; color:#000;">' +
                '<center>' +
                    '<h1 style="margin:0;">Meghana Physio Care & Home Services</h1>' +
                ...
                '<div style="margin-top:80px; text-align:right; padding-right: 20px;">' +
                    ...
                    '<p style="margin-top:5px; font-weight:bold; font-size:24px;">Meghana P</p>' +
    ...
    function openBillingModal(p) {
        ...
        // ESCAPED BACKTICKS AND VARIABLES
    ...
    function calculateGrandTotal() {
        ...
        if (grandTotalElement) grandTotalElement.innerText = grandTotal;
    }

    function printFinalBill() {
        if (!currentBillPatient) { alert("Error: No patient data found."); return; }
        ...
        rows.forEach(function(tr) {
            ...
            if (service) {
                tableRowsHtml += "<tr><td style='padding:10px; border:1px solid #ddd;'>" + service + "</td><td style='padding:10px; border:1px solid #ddd; text-align:center;'>" + amount + "</td><td style='padding:10px; border:1px solid #ddd; text-align:center;'>" + sessions + "</td><td style='padding:10px; border:1px solid #ddd; text-align:right;'>" + total + "</td></tr>";
            }
        });
        ...
        var htmlContent = "<div style='padding:40px; font-family:sans-serif; color:#333; line-height:1.5;'>" +
            "<center><h1 style='margin:0; color:#1a365d;'>Meghana Physio Care & Home Services</h1><p style='margin:5px 0;'>Specialized Physiotherapy Rehabilitation</p><h3 style='text-decoration:underline; margin-top:15px;'>Invoice</h3></center>" +
            "<div style='display:flex; justify-content:space-between; margin-top:30px; border-bottom:1px solid #eee; padding-bottom:15px;'>" +
            "<div><p><b>Patient Name:</b> " + p.name + "</p><p><b>Patient ID:</b> " + (p.patient_id || 'N/A') + "</p><p><b>Mobile Number:</b> " + (p.mobile || 'N/A') + "</p></div>" +
            "<div style='text-align:right;'><p><b>Bill No:</b> " + (p.billNo || 'N/A') + "</p><p><b>Date:</b> " + new Date().toLocaleDateString() + "</p><p><b>Status:</b> " + paymentStatus.toUpperCase() + "</p></div></div>" +
            "<table style='width:100%; border-collapse:collapse; margin-top:25px;'><thead><tr style='background:#f8fafc;'><th style='padding:10px; border:1px solid #ddd; text-align:left;'>Services Rendered</th><th style='padding:10px; border:1px solid #ddd;'>Rate</th><th style='padding:10px; border:1px solid #ddd;'>Sessions</th><th style='padding:10px; border:1px solid #ddd; text-align:right;'>Total (?)</th></tr></thead>" +
            "<tbody>" + tableRowsHtml + "</tbody><tfoot><tr style='font-weight:bold; background:#fdfdfd;'><td colspan='3' style='padding:12px; border:1px solid #ddd; text-align:right;'>Grand Total:</td><td style='padding:12px; border:1px solid #ddd; text-align:right;'>"+grandTotal+"</td></tr></tfoot></table>" +
            "<div style='margin-top:60px; display:flex; justify-content:space-between;'><div style='font-size:12px; color:#666;'><p><i>* Registered Physiotherapy Services</i></p><p><i>Issued by: Meghana Physio Care Administration</i></p></div><div style='text-align:center;'><p style='margin-bottom:40px;'><b>Authorized Signatory</b></p><p style='font-family:serif; font-size:22px; margin:0;'>Meghana P</p><p style='margin:0; font-size:14px;'>(Consultant Physiotherapist)</p></div></div></div>";
    ...
    async function updatePaymentDetails() {
        ...
        let totalSessions = 0;
        document.querySelectorAll('.session-qty').forEach(input => totalSessions += parseInt(input.value) || 0);
    ...
    function showDetails(p) {
        ...
        if (isLocked) {
            ...
            saveButton.innerText = "RECORD LOCKED";
        } else {
            formInputs.forEach(input => {
        ...
        area.innerHTML = \`
            <div style="padding:40px; font-family: sans-serif; color:#000;">
                ...
                <table style="width:100%; margin-top:20px; border-collapse: collapse; line-height: 2;">
                    <tr><td style="width:50%;"><strong>Patient ID:</strong> \${p.patient_id}</td><td style="width:50%; text-align:right;"><strong>Print Date:</strong> \${new Date().toLocaleDateString()}</td></tr>
                    <tr><td><strong>Name:</strong> \${p.name}</td><td><strong>Mobile:</strong> \${p.mobile}</td></tr>
                    <tr><td><strong>Age Group:</strong> \${p.age}</td><td><strong>DOB:</strong> \${p.dob ? new Date(p.dob).toLocaleDateString() : 'N/A'}</td></tr>
                    <tr><td colspan="2"><strong>Address:</strong> \${p.address}</td></tr>
                    <tr><td colspan="2"><strong>Primary Issue:</strong> \${cleanedIssue}</td></tr>
                    <tr><td colspan="2"><strong>Treatment Status:</strong> \${p.status || 'Patient Considered'}</td></tr>
                </table>
                ...
                <div style="margin-top:80px; text-align:right; padding-right: 20px;">
                    ...
                    <p class="signature-text" style="margin-top:5px; font-weight:bold; font-size:24px;">Meghana P</p>
                    <p style="margin-top:-5px; font-size: 12px; color: #555;">(Consultant Physiotherapist)</p>
                </div>
            </div>
        \`;
    ...
    async function handlePasswordUpdate(e) {
        ...
        try {
            ...
            if (response.ok) { alert("? " + resultText); closePasswordModal(); location.reload(); } 
            else { alert("? Error: " + resultText); }
    ...
    async function openSessionsModal(encodedData) {
        ...
        document.getElementById('sess_patient_info').innerText = "Sessions for: " + p.name;
        ...
        try {
            ...
            if(sessions.length === 0) { 
                ...
                sessions.forEach(s => {
                    addSessionRow(s.session_date, s.session_details); 
                });
            }
        } catch(err) { console.error("Error loading sessions:", err); }
    }

    function addSessionRow(dateStr = '', details = '') {
        const tbody = document.getElementById('sessionsTableBody');
        ...
        // ESCAPED BACKTICKS AND VARIABLES FOR NODE.JS
        tr.innerHTML = \`
            <td style="padding:10px; border:1px solid #ddd; text-align:center;">\${rowCount}</td>
            <td style="padding:10px; border:1px solid #ddd;">
                <input type="date" class="sess-date" value="\${cleanDate}" style="width:90%">
            </td>
            <td style="padding:10px; border:1px solid #ddd;">
                <textarea class="sess-desc" style="width:95%; height:40px;">\${details || ''}</textarea>
            </td>
            <td style="padding:10px; border:1px solid #ddd; text-align:center;">
                 <button onclick="removeSessionRow(this)" style="background:red; color:white; border:none; padding:5px 10px; cursor:pointer;">X</button>
            </td>
        \`;
    ...
    async function saveSessions() {
        ...
        const sessions = [];
        rows.forEach((tr, i) => {
            const dateVal = tr.querySelector('.sess-date').value;
            const descVal = tr.querySelector('.sess-desc').value;
            if(dateVal || descVal) {
                sessions.push({
                    session_number: i + 1,
                    session_date: dateVal,
                    session_details: descVal
                });
            }
        });
        try {
            ...
            if(res.ok) {
                ...
                document.getElementById('sessionsModalOverlay').style.display = 'none';
            } else { alert("Error saving sessions"); }
        } catch(err) { alert("Network error while saving."); }
    }

    function printSessions() {
        var area = document.getElementById('printArea');
        var patientNameElement = document.getElementById('sess_patient_info');
        var patientName = patientNameElement ? patientNameElement.innerText : 'N/A';
        
        // Clone the table to avoid modifying the original
        var originalTable = document.querySelector('#sessionsModalOverlay table');
        ...
        for (var i = 0; i < rows.length; i++) {
            rows[i].deleteCell(-1);
            // Replace input/textarea with text for printing
            var dateInput = rows[i].cells[1].querySelector('input');
            if(dateInput) rows[i].cells[1].innerText = dateInput.value;
            
            var descInput = rows[i].cells[2].querySelector('textarea');
            if(descInput) rows[i].cells[2].innerText = descInput.value;
        }
        ...
        area.innerHTML = \`
            <div style="padding:40px; font-family: sans-serif; color:#000;">
                <center>
                    <h1 style="margin:0;">Meghana Physio Care & Home Services</h1>
                    <p style="margin:5px 0;">Specialized Physiotherapy Rehabilitation</p>
                    <h2 style="margin:20px 0; border-bottom:2px solid #000; padding-bottom:10px;">SESSION TRACKER REPORT</h2>
                </center>
                <div style="margin-bottom:20px;">
...
// ADD THIS ROUTE AT THE BOTTOM OF module.exports
    ...
    const adminEmail = 'meghanap142@gmail.com'; 
